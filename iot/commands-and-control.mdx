...
description: "Send commands back to devices over MQTT, OPC-UA, or Modbus -- with single-fire guarantees, retries, and result tracking."
icon: "paper-plane"
---
...
<Warning>
  Commands are real actions on real hardware. Treat them like database writes -- always include a single-fire key and consider human-approval gates for high-stakes actions.
</Warning>
...
sequenceDiagram
    ...
    participant Device
    participant Result as Command result

    Pipeline->>IoT: command (device_id, action, args, single-fire key)
    IoT->>IoT: Single-fire check (24h window)
    IoT->>Device: Native protocol frame
    Device-->>IoT: ACK or error
    IoT->>Result: Result event (success / failure / timeout)
    Result->>Pipeline: Downstream listeners react
```

Every command carries a `user_id`, a single-fire key, and a correlation ID that links it to the result event.
...
|---|---|
| **Single-fire** | Same single-fire key within 24 hours = no double-fire. The first result is returned. |
| **Timeouts** | Default 30 seconds. Configurable per command. After timeout, a `command.timeout` event is emitted. |
| **Retries** | Up to 3 retries with automatic backoff for transient protocol errors. Application-level errors (e.g. invalid Modbus address) are not retried. |
