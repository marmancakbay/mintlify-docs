...
DEHA ONE's IoT layer is the bridge between the physical world and your agents and pipelines. It speaks the protocols industrial devices already speak -- **MQTT, OPC-UA, Modbus, HTTP polling, and inbound webhooks** -- and turns their telemetry into events your platform can react to.
...
<CardGroup cols={2}>
  ...
  <Card title="Send commands" icon="paper-plane">
    Trigger actuators, write Modbus coils, call OPC-UA methods, or push MQTT messages back to devices -- with single-fire guarantees and result timeouts.
  </Card>
  ...
  <Card title="Update device configs live" icon="rotate">
    Add, remove, or reconfigure devices -- live, no restart. Existing connections rebind to the new configuration automatically.
  </Card>
  <Card title="Trigger pipelines from devices" icon="bolt">
    Every telemetry reading becomes an event. Match it in your pipeline's trigger configuration and run any pipeline building block in response.
  </Card>
</CardGroup>
...
Each connection runs independently with automatic reconnection, per-device rate limiting, and isolated handling of messages that fail validation. See [Protocols](/iot/protocols) for setup details.
...
sequenceDiagram
    participant Device
    participant IoT as IoT layer
    participant Schema
    participant Events as Telemetry events
    participant Pipeline

    Device->>IoT: Raw frame (MQTT / OPC-UA / Modbus / HTTP / webhook)
    IoT->>Schema: Validate fields & types
    Schema-->>IoT: ok / flagged
    IoT->>Events: Publish event (user + device id)
    Events->>Pipeline: Trigger (match on event type / source)
    Pipeline->>Pipeline: Run steps (transform, query, agent, optimize, etc.)
```

Once the telemetry is flowing, **anything in the platform can react to it** -- alerts via Slack, anomaly detection, forecasting, dashboard updates, agent escalation, or commands back to the device.
...
See [Devices & Schemas](/iot/devices-and-schemas) for the configuration reference and validation rules.
...
Every command carries a single-fire guarantee (so retries do not double-fire actuators), a result timeout, and emits a result event for downstream listeners. See [Commands & Control](/iot/commands-and-control).
...
<AccordionGroup>
  ...
  <Accordion title="Energy demand forecasting">
    OPC-UA polls meter readings every minute. A forecasting pipeline generates next-day demand. The result feeds an optimization pipeline that schedules generation assets.
  </Accordion>
  ...
  <Accordion title="Smart-building HVAC">
    MQTT subscribes to building sensors. A pipeline triggers OPC-UA writes back to the BMS to adjust setpoints based on occupancy and outdoor weather pulled from your data.
  </Accordion>
</AccordionGroup>
