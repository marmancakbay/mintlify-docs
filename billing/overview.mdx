...
| **Document ingestion** | 10 credits per document | Knowledge-base parsing + indexing for search |
| **Semantic search** | 1 credit | Knowledge-base search-index query with relevance ranking |
| **Web search** | 3 credits per query | Multi-provider parallel fan-out + dedup + relevance ranking |
...
If you have your own API keys for LLM and semantic-index providers (OpenAI, Anthropic, Google Vertex AI with your GCP service account, Google Gemini, DeepSeek, etc.), you can connect them to DEHA ONE.
...
Semantic-index BYOK works the same way, point a collection at your own semantic-index provider, and ingestion / search consume zero platform credits.
