...
Automations are multi-step workflows that run automatically when something happens in your workspace. They connect AI agents, channels, data sources, optimization, web search, IoT, and business logic into repeatable processes that work around the clock.
...
<Info>
  **Automation-first**: every automation is also a callable endpoint. Once deployed, your automation becomes accessible via REST API (with its own API key, rate limit, and OpenAPI spec) and can be invoked from any agent, any other automation, or any external system. See [Dynamic API](/api-reference/dynamic-api-overview).
</Info>

<Info>
  Automations are **durable**. Failures, restarts, and timeouts are handled automatically with retries, failure recovery, and rollback — side-effect steps (REST writes, data loads, publishes, IoT commands) can be reversed if the automation aborts mid-flight.
</Info>
...
| **Event** | Match on any platform event | A new IoT telemetry frame from sensor X |
