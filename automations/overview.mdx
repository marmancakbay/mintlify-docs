...
Automations are multi-step workflows that run automatically when something happens in your workspace. They connect AI agents, channels, data sources, optimization, web search, and business logic into repeatable processes that work around the clock.
...
<Info>
  Automations are **durable**. Failures, restarts, and timeouts are handled automatically with retries, failure recovery, and rollback — side-effect steps (REST writes, data loads, publishes) can be reversed if the automation aborts mid-flight.
</Info>
...
| **Event** | Match on any platform event | A customer message arrives on any channel |
