...
DEHA ONE supports seven problem types out of the box, each backed by an industrial-grade solver tuned for that family. You pick the type that fits your problem; the solver, visualization, and summary are wired up automatically.
...
| Aspect | Detail |
|---|---|
| **Method** | Linear programming (Simplex / Barrier) |
...
| Aspect | Detail |
|---|---|
| **Method** | Mixed-integer programming (branch-and-bound) |
...
| Aspect | Detail |
|---|---|
| **Method** | Constraint solver with guided local search |
...
| Aspect | Detail |
|---|---|
| **Method** | Optimal assignment matching |
...
| Aspect | Detail |
|---|---|
| **Method** | Constraint programming with symmetry breaking |
...
| Aspect | Detail |
|---|---|
| **Method** | Minimum-cost network flow |
| **Speed** | Linear in problem size for typical networks |
| **Typical problems** | Supply chain, telecom routing, energy grid, transportation networks |

**Input:** supply and demand at each location, plus the capacity and unit cost of each route.
**You get back:** flow on every route, total cost, infeasibility report if the problem is over-constrained.
...
- **Validate the input.** Bad data is the most common cause of bad solutions, use [data quality](/data/data-quality) to clean and validate first.
