...
flowchart LR
    ...
    Broker --> Gateway[DEHA ONE<br/>secure ingest]
    Gateway --> Schema[Validate schema]
    Schema --> Stream[Telemetry feed]
...
DEHA generates the configuration, vaults the password, and deploys it. The platform opens the MQTT connection live -- no restart.

The generated configuration looks roughly like:
...
Deploy this via DEHA or the dashboard. The device is now expected -- frames matching the schema flow in; frames that don't are tagged and quarantined for review.
...
3. If frames are coming in but failing schema, the **Rejected** tab will show why (missing required field, wrong type, out-of-range value)
...
The platform deduplicates by `idempotency_key`, so retries do not double-fire the actuator. The command result is reported back to your pipeline and the device's command history.
...
- **Monitor the Rejected tab** in the IoT dashboard. If a device suddenly starts dumping rejected frames, the schema may have drifted (firmware change, hardware swap).
