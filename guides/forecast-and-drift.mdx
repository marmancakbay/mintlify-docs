...
description: "Generate a forecast, register a model in the model registry, run predictions in production, and monitor for drift."
icon: "chart-line"
---
...
flowchart LR
    ...
    Train --> Register[Register in model registry]
...
> "Train a daily revenue forecast from the `sales` table, target column `total`, time column `order_date`. Register the model as `revenue_forecast` in the model registry."
...
The platform auto-selects the best-fit forecasting model, from fast statistical methods to neural approaches, based on the signal in your data.
...
1. **Model Registry** → `revenue_forecast` → version 1
...
- id: notify_drift
  ...
  attachments:
    - artifact_path: ${drift.report_html}
```
...
A detailed **HTML drift report** is attached for deep inspection.
