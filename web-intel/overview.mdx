...
- **Search** — fan a query out to many leading search providers in parallel and get one deduplicated, re-scored result set
...
```mermaid
flowchart LR
    Search[Search many providers] --> Dedup[Deduplicate & rerank]
...
- **Browser fleets** — clean content extraction runs in a managed browser pool
