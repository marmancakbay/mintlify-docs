...
<Info>
  All renders are user-scoped, cached by content, and stored in a per-user artifact store. Sharing is via signed iframe links that expire and are revokable.
</Info>
...
<CardGroup cols={2}>
  <Card title="Chart from plain English" icon="sparkles">
    "Plot monthly revenue for the last 12 months as a line chart with a 3-month moving average." DEHA writes the chart spec and renders it.
  </Card>
  <Card title="Chart from a spec" icon="code">
    Hand-author interactive chart specs (Vega-Lite or Plotly) for full control. The renderer normalizes either input.
  </Card>
  ...
  <Card title="Embed anywhere" icon="square-arrow-up-right">
    Signed iframe URLs embed charts and dashboards in your internal tools — without exposing data or credentials.
  </Card>
  ...
  <Card title="Fleet PDF reports" icon="file-lines">
    Multi-page health reports across hundreds of users, teams, or business units — auto-archived per user.
  </Card>
</CardGroup>
...
```mermaid
flowchart LR
    Source[Data source<br/>SQL / pipeline / agent] --> Spec[Chart spec]
    Spec --> Render[Server-side renderer]
    Render --> Cache[Render cache<br/>user + spec + format]
    Cache --> Out[PNG / SVG / PDF / HTML iframe]
```

- **Single charts** render in a fast, browser-free path.
- **Dashboards and PDF reports** render in a managed browser pool with templated layouts.
- Outputs are cached by the spec, format, and dimensions — repeated requests are instant.
...
- Diff its definition across versions (full audit trail with versioning and rollback)
...
1. Resolves the data source (your warehouse)
...
```
https://embed.deha.one/c/{chart_id}?token=<signed-token>
```
